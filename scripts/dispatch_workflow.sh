...
if [[ "$enable_javascript" != "true" && "$enable_javascript" != "false" ]]; then
  echo "--enable-javascript must be true or false" >&2
  exit 1
fi

if [[ "$local_content_path" = /* ]] || [[ "$local_content_path" == *".."* ]]; then
  echo "--local-content-path must stay inside assets" >&2
  exit 1
fi

if [[ "$content_mode" == "HTML" && "$local_content_path" != *.html ]]; then
  echo "HTML mode expects .html path" >&2
  exit 1
fi
if [[ "$content_mode" == "PDF" && "$local_content_path" != *.pdf ]]; then
  echo "PDF mode expects .pdf path" >&2
  exit 1
fi
